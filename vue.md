...
vue数据双向绑定是通是通过数据劫持结合发布-订阅模式的方式来实现的

过程：
  
  首先要对数据进行劫持监听，所以我们需要设置一个监听器Observer（观察者），用来监听所有属性。

  如果属性发生了变化，就需要通知订阅者Watcher看是否需要更新。
  
  因为订阅者是有很多个，所以我们需要有一个消息订阅器Dep来专门收集这些订阅者，然后在监听器Observer和订阅者Watcher之间进行同一管理
  
  还需要一个指令解析器Compile，对每个节点元素进行扫描和解析，将相关执行（v-model，v-on）对应初始化成一个订阅者Watcher，并替换模版数据或者绑定相应的函数，
